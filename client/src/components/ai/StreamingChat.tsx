...
export function StreamingChat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call streaming endpoint using apiClient for proper authentication
      const responseBody = await apiClient.streamChat(input)
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-2 p-4 border-b bg-background">
        ...
        <h2 className="text-lg font-semibold">Streaming AI Chat</h2>
        <span className="text-xs text-muted-foreground ml-auto">
          Real-time with rich content
        </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t bg-background">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask for code examples, suggestions, or anything..."
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Real-time streaming - watch AI type responses
        </p>
      </form>
    </div>
  )
